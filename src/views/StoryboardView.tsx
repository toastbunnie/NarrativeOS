...
import { Clapperboard, Film, ChevronRight, Search } from 'lucide-react';
import { useApp } from '../context/AppContext';
import { QuestStoryboardView } from '../components/quests/QuestStoryboardView';

export const StoryboardView: React.FC = () => {
  const { t, quests, storyboards, activeProjectId, refreshData } = useApp();
  ...
  const storyboardCountFor = (questId: string) =>
    storyboards.filter((sb) => sb.questId === questId).length;
    ...
    <div className="p-6 space-y-4">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>
              分镜脚本 / Project-level Storyboard · 关联任务与任务步骤
            </p>
          </div>
        </div>
      </div>
      ...
      {!selectedQuest ? (
        <div className="glass-card p-4 space-y-3">
          ...
          {projectQuests.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {projectQuests.map((q) => {
                const count = storyboardCountFor(q.id);
                return (
                  <button
                    key={q.id}
                  ...
                  >
                    <div className="flex items-start space-x-3 min-w-0">
                      <div className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: 'var(--bg-tertiary)' }}>
                        <Film className="w-4 h-4" style={{ color: 'var(--accent-primary)' }} />
                      </div>
        ...
        <div className="space-y-3">
          ...
          {/* Delegate to existing quest-coupled storyboard editor */}
          <QuestStoryboardView quest={selectedQuest} onRefresh={refreshData} />
        </div>
      )}
    </div>
  );
